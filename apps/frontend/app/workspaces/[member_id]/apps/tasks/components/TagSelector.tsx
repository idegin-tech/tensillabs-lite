...
export function TagSelector({
    ...
    const [hasSelectionChanges, setHasSelectionChanges] = useState(false)
    ...
    const handleSelect = (tagValue: string) => {
        ...
        const newSelectedTags = isSelected
            ? localSelectedTags.filter(t => t !== tagValue)
            : [...localSelectedTags, tagValue]

        setLocalSelectedTags(newSelectedTags)
        setHasSelectionChanges(true)
    }
                    ...
                    {localSelectedTags.length > 0 ? (
                        <div className="flex gap-1 items-center max-w-full overflow-hidden">
                            ...
                            {remainingCount > 0 && (
                                <Badge
                                    variant="outline"
                                    className="text-xs border-0 bg-card"
                                >
                                    +{remainingCount}
                                </Badge>
                            )}
