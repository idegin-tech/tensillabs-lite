const fs = require('fs');
const path = require('path');

function createIconPNG(size, filename) {
  const canvas = `<svg width="${size}" height="${size}" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
  <rect width="512" height="512" fill="#000000" rx="80"/>
  <text x="256" y="280" dominant-baseline="central" text-anchor="middle" fill="white" font-family="Arial, sans-serif" font-size="200" font-weight="bold">TL</text>
</svg>`;
  
  fs.writeFileSync(path.join(__dirname, '..', 'public', filename), canvas);
  console.log(`Created ${filename}`);
}

createIconPNG(192, 'icon-192x192.svg');
createIconPNG(512, 'icon-512x512.svg');

console.log('Icon generation complete');
